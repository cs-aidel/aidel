...
int main(int argc, char** argv) {
    ...
    // aidel using sorted_list
    printf("Running aidel with sorted_list\n");
    rng.seed(seed);
    for (int t = 0; t < num_trials; t++) {
        for (int &query : queries) {
            query = keys[dist(rng)];
        ...
        auto start = clock();
        for (const int& key : queries) {
            int pos = aidel.findInsert(key);
        }
        double elapsed = double(clock() - start) / CLOCKS_PER_SEC;
        times_ai_sl.push_back(elapsed);
    }
    ...
    time_ai_sl = 1e+9 * std::accumulate(times_ai_sl.begin(), times_ai_sl.end(), 0.) / (num_trials * QUERIES_PER_TRIAL);
    ...
    printf("%8.1lf ns : %.40s\n", time_ai_sl, "aidel with sorted_list");
    ...
    // aidel with sorted_list
    printf("Inserting aidel sorted_lists\n");
    for (int t = 0; t < num_trials; t++) {
        auto start = clock();
        //for (const int& key : keys_ooo) {
        for(int i = 0; i<insert_number; i++) {
    ...
    time_ai_sl = 1e+9 * std::accumulate(times_ai_sl.begin(), times_ai_sl.end(), 0.) / (num_trials * insert_number);
    ...
    // aidel with sorted_list
    printf("Running aidel sorted_list\n");
    rng.seed(seed);
    for (int t = 0; t < num_trials; t++) {
        for (int &query : queries) {
            query = keys_total[dist_total(rng)];
